...
export const AI_CODE_STATS_ORGANIZATION_OPTIONS_PATH = "/api/v1/ai-code-stats/organization-options"
...
const resolveIngestPath = (pathname: string, targetPath: string): string => {
	...
	for (const knownPath of [
		AI_CODE_STATS_INGEST_PATH,
		AI_CODE_STATS_COMMIT_STATUS_PATH,
		AI_CODE_STATS_ORGANIZATION_OPTIONS_PATH,
		AI_TOKEN_USAGE_INGEST_PATH,
	]) {
...
export const resolveAiCodeStatsOrganizationOptionsUrl = (rawValue: string): string => {
	...
	parsedUrl.pathname = resolveIngestPath(parsedUrl.pathname, AI_CODE_STATS_ORGANIZATION_OPTIONS_PATH)
	parsedUrl.hash = ""
